...
BEST_CHECKPOINT  = CHECKPOINT_DIR / "te-executor-v5b-best.pt"
...
def _step(batch, model, device, chord_tone_tensor, pitch_fn, dur_fn, rest_fn):
    chord_ids     = batch["chord_ids"].to(device, non_blocking=True)
    chord_mask    = batch["chord_mask"].to(device, non_blocking=True)
    phrase_id     = batch["phrase_id"].to(device, non_blocking=True)
    ...
    pos_in_phrase = batch["pos_in_phrase"].to(device, non_blocking=True)
...
def _train_epoch(model, loader, optimizer, scaler, device, chord_tone_tensor,
...
def main():
    ...
    train_loader = DataLoader(
        ...
        collate_fn=collate_note_window, num_workers=4, pin_memory=True,
    )
    ...
    wandb.init(
        ...
        name="note-executor-v5b-runpod",
        config={
            "d_model": D_MODEL, "nhead": NHEAD, "num_layers": NUM_LAYERS,
            "dim_feedforward": DIM_FEEDFORWARD, "dropout": DROPOUT,
            "label_smoothing": 0.1, "harmonic_weight": HARMONIC_WEIGHT,
            "lr": LR, "scheduler": "warmup+cosine",
            "warmup_epochs": warmup_epochs, "T_max": EPOCHS - warmup_epochs,
            "eta_min": 1e-5, "weight_decay": WEIGHT_DECAY,
    ...
    torch.save(model.state_dict(), CHECKPOINT_DIR / "note_executor_v5b_final.pt")
